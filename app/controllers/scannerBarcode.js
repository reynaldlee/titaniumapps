...
var openScanner = function() {
	...
	picker.init("s8CrSgZUEeSXBke7UTTIqsdeu8dWgeFymJzoTh7tgfY", 0);
	...
	picker.setSuccessCallback(function(e) {
		...
		var labelDetail = Ti.UI.createLabel({
			height : Ti.UI.SIZE,
			widht : Ti.UI.FILL,
			text : " (" + e.symbology + ") : " + e.barcode,
		});
		detailView.addEventListener("click", function(e) {
			picker.remove(detailView);
		});
		detailView.add(labelDetail);
		picker.add(detailView);
...
$.barcodeGeneratorButton.addEventListener('click', function() {
	var controller = Alloy.createController('barcodeGenerator');
	var win = controller.getView();
	win.open();
});
$.qrCodeGeneratorButton.addEventListener('click', function() {
	var controller = Alloy.createController('qrcodeGenerator');
	var win = controller.getView();
	win.open();
});
var rootWindow = $.mainwindow;
//rootWindow.add($.scannerButton);
rootWindow.open();
